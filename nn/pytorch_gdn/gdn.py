...
class GDN(nn.Module):
    ...
    def forward(self, inputs):
        with warnings.catch_warnings():
            ...
            unfold = False
            # print("In GDN forward, input dimensions are: ", inputs.size())
            if inputs.dim() == 5:
                unfold = True
                bs, ch, d, w, h = inputs.size()
                inputs = inputs.view(bs, ch, d*w, h)
            
            #Added to find ch of 3D input (Batch, channel, features)
            elif inputs.dim() == 4:
            ...
            if inputs.dim() in [4,5]:
                ...
                norm_ = nn.functional.conv1d(inputs**2, gamma, beta)
                norm_ = torch.sqrt(norm_)
